...
import { adjustStock } from '../../../services/inventoryService'; // Import the adjustStock function
...
export default function AdjustInventoryPage() {
  ...
  const [formData, setFormData] = useState({
    ...
    notes: '', // Notes are required for adjustments
  ...
  const handleChange = (e) => {
    const { name, value } = e.target;
     ...
     // Allow typing '-' sign for negative adjustments in quantity field
    ...
    // Ensure quantity is a valid number or empty string
     if (name === 'quantity' && value !== '' && isNaN(parseFloat(value))) {
         // Optionally show a temporary message here, but preventing input is simpler
         return; // Prevent typing non-numeric characters (except initial '-')
     }
  ...
  const handleSubmit = async (e) => {
    ...
    // Check if required fields are filled and quantity is a valid number (can be negative) but not zero
    if (!formData.product_id || !formData.location_id || isNaN(adjustmentQuantity) || adjustmentQuantity === 0) {
         setFormError('Por favor, complete los campos Producto, Ubicación y Cantidad con una cantidad de ajuste válida (positiva o negativa, pero no cero).');
         return; // Stop submission if validation fails
     }
      // Notes are mandatory for adjustments
      if (!formData.notes.trim()) {
          setFormError('Por favor, ingrese el motivo del ajuste en el campo Notas.');
          return; // Stop submission if notes are empty
      }
    ...
    try {
        // Call the adjustStock service function with prepared data
        const response = await adjustStock({
            ...
            // reference_number is optional and included if present in formData
        });
        ...
        setFormError(err.message || 'Ocurrió un error inesperado al registrar el ajuste.');
         // Removed alert for errors, using formError state instead
    } finally {
        ...
        <div className="card shadow-sm"> {/* Added shadow */}
            <div className="card-body">
                ...
                <form onSubmit={handleSubmit}>
                    ...
                    <div className="row g-3 mb-3">
                        <div className="col-md-6"> {/* col-md-6 takes half width on medium+ screens */}
                         ...
                         <div className="col-md-6">
                            <label htmlFor="locationSelect" className="form-label">Ubicación <span className="text-danger">*</span></label> {/* Added required indicator */}
                            <select
                                className="form-select"
                                id="locationSelect"
                                name="location_id"
                    ...
                    <div className="mb-3">
                         <label htmlFor="quantityInput" className="form-label">Cantidad a Ajustar (+ para aumentar, - para disminuir) <span className="text-danger">*</span></label> {/* Added required indicator */}
                        <input
                            type="number"
                            ...
                            onChange={handleChange}
                            required // HTML5 required attribute
                            // No min/max here as it can be positive or negative
                            disabled={isSubmitting} // Disable while submitting
                        />
                        <div className="form-text">
                           Ingrese la cantidad exacta para ajustar el stock (ej: 5 para añadir, -3 para remover). No el nuevo total. Debe ser diferente de cero.
                        </div>
                    </div>
                    ...
                    <div className="mb-3">
                        ...
                        <input
                            ...
                            disabled={isSubmitting} // Disable while submitting
                        />
                         <div className="form-text">Número de referencia opcional para el ajuste.</div> {/* Added helper text */}
                    </div>


                    <div className="mb-4"> {/* Increased bottom margin */}
                        <label htmlFor="notesTextarea" className="form-label">Motivo del Ajuste (Notas) <span className="text-danger">*</span></label> {/* Added required indicator */}
                        <textarea
                            className="form-control"
                            id="notesTextarea"
                            rows="3"
                            ...
                            onChange={handleChange}
                            required // Notes are mandatory for adjustments
                            disabled={isSubmitting} // Disable while submitting
                        ></textarea>
                         <div className="form-text">Explique detalladamente el motivo de este ajuste (ej: inventario físico, daño, pérdida).</div> {/* Added helper text */}
                    </div>

                    {/* Submit button */}
                    <button
                        type="submit"
                        className="btn btn-warning w-100" // Full width button, warning color for adjustments
