...
import { transferStock } from '../../../services/inventoryService'; // Import the transferStock function
...
export default function TransferInventoryPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.product_id || !formData.from_location_id || !formData.to_location_id || isNaN(quantityValue) || quantityValue <= 0) {
         setFormError('Por favor, complete los campos Producto, Ubicación de Origen, Ubicación de Destino y Cantidad con valores válidos (Cantidad > 0).');
     ...
     if (formData.from_location_id === formData.to_location_id) {
         ...
         return; // Stop submission if locations are the same
     }
    ...
    try {
        // Call the transferStock service function with prepared data
        const response = await transferStock({
            ...formData, // Include other form data (product_id, from_location_id, to_location_id, user_id, notes)
            quantity: quantityValue, // Ensure quantity is sent as a number (positive)
            // notes is optional and included if present in formData
        });
        ...
        if (response && response.success) {
            ...
            // Redirect to the transfers history report page on success
            router.push('/reports/transfers'); // Redirect to transfers report
        } else {
        ...
        setFormError(err.message || 'Ocurrió un error inesperado al registrar la transferencia.');
         // Removed alert for errors, using formError state instead
    } finally {
                ...
                <form onSubmit={handleSubmit}>
                     <div className="mb-3">
                        ...
                        >
                         ...
                         <div className="form-text">Seleccione el producto a transferir.</div> {/* Added helper text */}
                    </div>

                    <div className="row g-3 mb-3">
                         <div className="col-md-6">
                            <label htmlFor="fromLocationSelect" className="form-label">Ubicación de Origen <span className="text-danger">*</span></label> {/* Added required indicator */}
                            <select
                                className="form-select"
                                ...
                                required // HTML5 required attribute
                                 disabled={isSubmitting} // Disable while submitting
                            >
                                <option value="">-- Seleccione ubicación de origen --</option> {/* Improved placeholder */}
                                {locations.map(location => (
                                    // Ensure location_id is the correct key from your API response
                                    <option key={location.id} value={location.id}>{location.name}</option>
                                ))}
                            </select>
                             <div className="form-text">Stock será deducido de esta ubicación.</div> {/* Added helper text */}
                        </div>

                         <div className="col-md-6">
                            <label htmlFor="toLocationSelect" className="form-label">Ubicación de Destino <span className="text-danger">*</span></label> {/* Added required indicator */}
                            <select
                                className="form-select"
                                id="toLocationSelect"
                                name="to_location_id"
                                ...
                                required // HTML5 required attribute
                                 disabled={isSubmitting} // Disable while submitting
                            >
                                <option value="">-- Seleccione ubicación de destino --</option> {/* Improved placeholder */}
                                {/* You might want to filter the destination location so it cannot be the same as the source */}
                                {locations
                                    // Optional: Filter out the selected from_location_id from the destination list
                                    // .filter(location => location.location_id !== parseInt(formData.from_location_id))
                                    .map(location => (
                                        // Ensure location_id is the correct key from your API response
                                        <option key={location.location_id} value={location.location_id}>{location.name}</option>
                                    ))}
                            </select>
                             <div className="form-text">Stock será añadido a esta ubicación.</div> {/* Added helper text */}
                        </div>
                    </div>


                    <div className="mb-3">
                         <label htmlFor="quantityInput" className="form-label">Cantidad a Transferir <span className="text-danger">*</span></label> {/* Added required indicator */}
                        <input
                            type="number"
                            ...
                            onChange={handleChange}
                            required // HTML5 required attribute
                            min="0.01" // Minimum positive value for transfer
                            disabled={isSubmitting} // Disable while submitting
                        />
                         <div className="form-text">Ingrese la cantidad exacta del producto a transferir (debe ser mayor a 0).</div> {/* Added helper text */}
                    </div>
                    ...
                    <div className="mb-4"> {/* Increased bottom margin */}
                        ...
                        <textarea
                            ...
                            onChange={handleChange}
                             disabled={isSubmitting} // Disable while submitting
                        ></textarea>
                         <div className="form-text">Detalles adicionales sobre la transferencia.</div> {/* Added helper text */}
                    </div>
